...
const Catalog = () => {
  ...
  const {loading,
        error,
        message,
        allBorrowedBooks}=useSelector((state) => state.borrow);
            ...
            <tbody>
              {
                booksToDisplay.map((book,index)=>(
                  <tr key={index} className={(index+1)%2===0?"bg-gray-50":""}>
                    ...
                    <td className="px-4 py-2 cursor-pointer">{book.returnDate?(<FaSquareCheck className="w-6 h-6"/>):(<PiKeyReturnBold onClick={()=>openReturnBookPopup(book.book,book?.user.email)} className="w-6 h-6"/>)}</td>
                  </tr>
                ))
              }
            </tbody>
