...
export const forgotPassword=catchAsyncErrors(async(req,res,next)=>{
    ...
    try {
        await sendEmail({
            email:user.email,
            subject:"BookHive Library Management System Password Recovery",
            message,
        })
        res.status(200).json({
            success:true,
...
export const resetPassword=catchAsyncErrors(async (req,res,next)=>{
    ...
    if(req.body.password.length<8||req.body.password.length>16||req.body.confirmPassword.length<8||req.body.confirmPassword.length>16 ){
        return next(new ErrorHandler("Password or confirm password must be between 8 and 16 characters.",400))
    }
